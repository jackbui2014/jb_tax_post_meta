...
class JB_Metabox {
     ...
     * @return void
     */
    public function __construct( $id, $title, $args = array(), $meta = array() ) {
        $this->id    = $id;
        $this->title = $title;
        $this->meta  = $meta;
    ...
    public function display( $post ) {
        ...
        if( isset($form_fields['0']['type']) && $form_fields['0']['type'] == 'html' ){
            echo $form_fields['0']['value'];
        }
        else {
    ...
    public function table( $rows, $formdata, $errors = array() ) {
        ...
        $output = JB_Forms::table_wrap( $output );
    ...
    public function display_notices( $notices, $class = 'updated' ) {
        ...
        foreach ( (array) $notices as $notice ) {
            echo JB__admin_notice( $notice, $class );
        }
    }
    ...
    protected function save( $post_id ) {
        $form_fields = $this->form_fields();
        $to_update   = JB_Forms::validate_post_data( $form_fields );
